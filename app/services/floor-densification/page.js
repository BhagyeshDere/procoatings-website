...
export default function FloorDensificationPage() {
  return (
    <main className="bg-white text-[#0F3250] overflow-hidden">
      ...
      {/* 🔵 QUICK SELECTOR GRID */}
      <section className="py-12 md:py-16 bg-[#0F3250] text-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {["Sodium", "Lithium", "Ashford", "Formula"].map((item, i) => (
              <div
                key={i}
                className="group bg-white/5 border border-white/10 rounded-xl md:rounded-2xl p-4 md:p-6 text-center backdrop-blur-sm hover:bg-[#EAA33F] transition-all cursor-default"
              >
                <p className="text-[#EAA33F] group-hover:text-[#0F3250] text-xl md:text-2xl font-black mb-1 italic transition-colors">
                  0{i + 1}
                </p>
                <h3 className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.1em] md:tracking-[0.2em] text-slate-300 group-hover:text-[#0F3250] transition-colors">
                  {item}
                </h3>
              </div>
            ))}
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
            {[
              ...
              <div key={i} className="flex flex-col gap-4 md:gap-6 p-2">
                {/* FIXED: Removed the md:size prop which caused the build error */}
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-[#EAA33F]/5 flex items-center justify-center border border-[#EAA33F]/20 shadow-sm">
                  <item.icon className="text-[#EAA33F]" size={28} />
                </div>
                <div>
                  <h4 className="font-black text-lg md:text-xl mb-2 md:mb-3 uppercase tracking-tight italic text-[#0F3250]">
                    {item.title}
                  </h4>
      ...
      <section className="pb-16 md:pb-24 pt-6 md:pt-10">
        <div className="max-w-5xl mx-auto px-6">
          <div className="rounded-[32px] md:rounded-[48px] bg-gradient-to-br from-[#0F3250] to-[#1a4a70] text-white p-8 md:p-20 text-center shadow-2xl relative overflow-hidden">
            <div className="absolute right-[-10%] top-[-10%] w-64 h-64 md:w-80 md:h-80 bg-[#EAA33F]/10 rounded-full blur-[60px] md:blur-[100px]" />
            
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-black relative z-10 uppercase italic leading-tight">
              Ready for <br />
              <span className="text-[#EAA33F] not-italic">Heavy-Duty</span> Performance?
            </h2>

            <p className="mt-6 md:mt-8 text-sm md:text-lg text-slate-300 relative z-10 max-w-xl mx-auto font-medium leading-relaxed">
              Consult with our technical engineers for custom industrial floor hardening solutions.
            </p>

            <Link href="/contact" className="relative z-10 inline-block mt-8 md:mt-12 w-full sm:w-auto">
              <button className="w-full sm:w-auto bg-[#EAA33F] px-8 md:px-10 py-4 md:py-5 rounded-xl md:rounded-2xl font-black text-[#0F3250] hover:scale-105 transition shadow-2xl uppercase tracking-widest text-[10px] md:text-xs active:scale-95">
                Request Proposal
              </button>
            </Link>
          </div>
